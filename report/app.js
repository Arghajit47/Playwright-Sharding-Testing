...
function getHtmlFiles(folderPath) {
  ...
  return content
    ...
    .reduce((acc, file) => {
      const filePath = path.relative(__dirname, path.join(folderPath, file)); // Adjust path to be relative to the root
      acc[filePath] = "file"; // Store relative path of files
      return acc;
    }, {});
}
...
fs.writeFileSync(
  path.join(rootFolder, "folderStructure.json"), // Ensure it's written to the root folder
